 ...
 *
 *  3) Run:  npm i express express-rate-limit sqlite3 nodemailer
 *           node server.js
 *
...
// ----- DB (sqlite for pending verifications) -----
const db = await open({ filename: './registrations.db', driver: sqlite3.Database });
await db.exec(`CREATE TABLE IF NOT EXISTS pending (
...
app.post('/api/register', limiter, async (req, res) => {
  try {
    ...
    const now = Date.now();
    await db.run('INSERT INTO pending (token, username, password, email, created_at) VALUES (?,?,?,?,?)',
...
app.get('/verify', async (req, res) => {
  try {
    ...
    const row = await db.get('SELECT * FROM pending WHERE token = ?', token);
    ...
    if (ageMin > CONFIG.TOKEN_TTL_MIN) {
      await db.run('DELETE FROM pending WHERE token = ?', token);
...
setInterval(async () => {
  ...
  try { await db.run('DELETE FROM pending WHERE created_at < ?', cutoff); } catch {}
}, 60 * 60 * 1000);
